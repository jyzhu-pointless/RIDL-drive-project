...
def lineplot(src, xcol, ycol, xlabel, ylabel, title, cbarlabel, trimmer):
    ...
    # print(data)
    subdata_1 = [entry for entry in data if entry[6] == "0" and float(entry[0]) <= 150]
    subdata_2 = [entry for entry in data if entry[6] == "0.25" and float(entry[0]) <= 150]
    subdata_3 = [entry for entry in data if entry[6] == "0.5" and float(entry[0]) <= 150]
    subdata_4 = [entry for entry in data if entry[6] == "0.75" and float(entry[0]) <= 150]
    subdata_5 = [entry for entry in data if entry[6] == "1" and float(entry[0]) <= 150]
    print("subdata_1:\n", subdata_5)
    ...
    ax.set_xticks([i+0.5 for i in linspace(0, x5_dim-1, 4)])  # This line creates six ticks on the x axis which are centered on the column they correspond to.
    ax.set_yticks([i+0.5 for i in linspace(0, y5_dim-1, 7)])  # This line creates six ticks on the y axis which are centered on the row they correspond to.
    ...
    ax.set_yticklabels([0,10000,20000,30000,40000,50000,60000])
    ...
    # ax.legend(["0.00","0.25","0.50","0.75","1.00"],loc="right")
    ...
    save_and_display(fig, f"{src}_SIT_lineplot.png")
...
def main():
    ...
    parser.add_argument('-y_label', '--Y_LABEL', default="Adult female population size", type=str, help="Label for the Y axis.")#Migration rate
