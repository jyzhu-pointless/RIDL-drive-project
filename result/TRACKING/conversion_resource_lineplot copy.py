...
def lineplot(src, xcol, ycol, xlabel, ylabel, title, cbarlabel, trimmer):
    ...
    # print(data)
    subdata_1 = [entry for entry in data if entry[6] == "0"]
    subdata_2 = [entry for entry in data if entry[6] == "0.25"]
    subdata_3 = [entry for entry in data if entry[6] == "0.5"]
    subdata_4 = [entry for entry in data if entry[6] == "0.75"]
    subdata_5 = [entry for entry in data if entry[6] == "1"]
    print("subdata_1:\n", subdata_5)
    ...
    ax.set_xticklabels([0,80,160,240,320])
    ...
    save_and_display(fig, f"{src}_SIT_lineplot_11.png")
...
def main():
    ...
    parser.add_argument('-x_label', '--X_LABEL', default="Generation", type=str, help="Label for the x axis.")#Low density growth rate
