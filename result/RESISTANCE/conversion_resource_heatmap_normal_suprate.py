...
def heatmap(src, xcol, ycol, zcol, xlabel, ylabel, title, cbarlabel, colormap, colormap_display):
    ...
    data = data[442:883]  # This trims off the header.
    print(data)
    ...
    all_x = [round(math.log10(float(entry[xcol])) - math.log10(5E-5),1) for entry in data]
    ...
    for entry in data:
        x = round(math.log10(float(entry[xcol])) - math.log10(5E-5),1)
        y = float(entry[ycol])
        z = float(entry[zcol])
        if (np.isnan(z)):
            z = -50.0
    ...
    ax.set_xticks([i+0.5 for i in linspace(0, x_dim-1, 5)])  # This line creates six ticks on the x axis which are centered on the column they correspond to.
    ax.set_yticks([i+0.5 for i in linspace(0, y_dim-1, 5)])  # This line creates six ticks on the y axis which are centered on the row they correspond to.
    ...
    ax.set_xticklabels(["$5\times 10^{-5}$","5e-4","5e-3","5e-2","5e-1"])
    ...
    hm1 = ax.pcolormesh(plot_data, cmap = colormap_display, rasterized=True, vmin=0, vmax=1)
    ...
    colorbar = cbfig.colorbar(hm1, cax=cbax, orientation='vertical')#horizontal vertical
    # plt.clim(0, 100) 
    #colorbar.set_label(f"{cbarlabel}")

    hm = ax.pcolormesh(plot_data, cmap = colormap, rasterized=True, vmin=-50, vmax=1)

    #save_and_display(fig, f"{src[:-4]}.png")
    save_and_display(fig, f"{src}_heatmap_normal.png")
    #save_and_display(cbfig, f"colorbar.png")
    save_and_display(cbfig, f"{src}_colorbar_glf_normal.png")
    plt.show()
...
def main():
    ...
    parser.add_argument('-src', '--source', default="suppression_rate.csv", type=str, help="CSV file to make heatmap from.")
    ...
    parser.add_argument('-x_label', '--X_LABEL', default="Germline resistance rate", type=str, help="Label for the x axis.")#Low density growth rate
    ...
    color_range_1 = twilight_shifted(linspace(0.5, 1, 20))
    # color_range_1 = magma(linspace(1, 0, 100))
    color_range_2 = Greys(linspace(0.5, 0.5, 1000))
