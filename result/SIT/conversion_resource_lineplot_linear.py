...
def lineplot(src, xcol, ycol, xlabel, ylabel, title, cbarlabel, trimmer):
    ...
    data = data[43:64]  # This trims off the header.
    print(data)
    ...
    # z_min = min(all_z)
    # z_max = max(all_z)
    plot_data = [0.0 for i in range(x_dim)]
    plot_data_x = [0.0 for i in range(x_dim)]
    ...
    for entry in data:
       ...
       if not np.isnan(y):
           ...
           y_coord = y
       plot_data_x[x_coord] = x_coord + 0.5
       plot_data[x_coord] = y_coord + 0.5

    fig = plt.figure(figsize=(8.7,8.8))  # Confure the desired figure size.
    ...
    ax.set_yticks([i+0.5 for i in linspace(0, y_dim-1, 4)])  # This line creates six ticks on the y axis which are centered on the row they correspond to.
    ...
    hm1 = ax.plot(plot_data_x, plot_data, '-o', color='m')
    # hm2 = ax.plot(plot_data_2x, plot_data_2, '-o')
    # ax.legend(["$\\beta = 6$","$\\beta = 10$"],loc="upper right")
    ...
    save_and_display(fig, f"{src}_SIT_lineplot_linear.png")
...
def main():
    ...
    parser.add_argument('-y','--Y_COL', default=4, type=int, help="The column of the entries in Y.")
    # parser.add_argument('-z','--Z_COL', default=3, type=int, help="The column of the entries in Z [i.e. the color].")
    parser.add_argument('-x_label', '--X_LABEL', default="Drop ratio", type=str, help="Label for the x axis.")#Low density growth rate
    parser.add_argument('-y_label', '--Y_LABEL', default="Weeks to population elimination", type=str, help="Label for the Y axis.")#Migration rate
