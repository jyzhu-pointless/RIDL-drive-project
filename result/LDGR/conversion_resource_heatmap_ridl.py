...
def heatmap(src, xcol, ycol, zcol, xlabel, ylabel, title, cbarlabel, colormap, colormap_display):
    ...
    data = data[316:631]  # This trims off the header.
    print(data)
    for i in range(len(data)):
        data[i] = data[i].split(',')
        data[i] = data[i][3:4] + data[i][1:2] + data[i][5:6]
    drivename = data[0][0]
    ...
    fig = plt.figure(figsize=(7.0,8.5))  # Confure the desired figure size.
    ...
    ax.set_xticks([i+0.5 for i in linspace(0, x_dim-1, 8)])  # This line creates six ticks on the x axis which are centered on the column they correspond to.
    ...
    save_and_display(fig, f"{src}_heatmap_ridl.png")
    #save_and_display(cbfig, f"colorbar.png")
    save_and_display(cbfig, f"{src}_colorbar_glf_ridl.png")
    plt.show()
...
def main():
    ...
    parser.add_argument('-x_label', '--X_LABEL', default="Low-density growth rate", type=str, help="Label for the x axis.")#Low density growth rate
