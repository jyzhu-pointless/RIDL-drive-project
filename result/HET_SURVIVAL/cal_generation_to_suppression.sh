...
echo "heterozygotes_survival_rate,resistance_rate,drop_ratio,drive_efficiency,low_density_growth_rate,density_growth_curve,generation_to_suppression,type"

for file in $files
do
    #  在 raw_data/ 中找到相应的 file
    #  按最后一行判断是否抑制了
    # generation,size,female_size,r2_freq,rate_dr,rate_has_drive,drive_efficiency,germline_resistance,drop_ratio,embryo_resistance,low_density_growth_rate,density_growth_curve
    ##! csv 表头：
    # 1 generation, 2 size,           3 female_size,      4 r2_freq,
    ...
    heterozygotes_survival_rate=`echo $file | cut -f 5 -d '_'`
    ...
    do
        rate_drive=`echo $row | cut -f 5 -d ','` # 需要改成第5列，因为杂合可能存活
    ...
    echo "$heterozygotes_survival_rate,$resistance_rate,$drop_ratio,$drive_efficiency,$low_density_growth_rate,$density_growth_curve,$generation,$type"
