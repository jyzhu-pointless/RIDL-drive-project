...
def heatmap(src, xcol, ycol, zcol, xlabel, ylabel, title, cbarlabel, colormap, colormap_display):
    ...
    for i in range(len(data)):
        data[i] = data[i].split(',')
        data[i] = data[i][0:1] + data[i][2:3] + data[i][6:7]
    drivename = data[0][0]
    ...
    ax.set_yticks([i+0.5 for i in linspace(0, y_dim-1, 6)])  # This line creates six ticks on the y axis which are centered on the row they correspond to.
    ...
    save_and_display(fig, f"{src}_heatmap_reces.png")
    #save_and_display(cbfig, f"colorbar.png")
    save_and_display(cbfig, f"{src}_colorbar_glf_reces.png")
    plt.show()
...
def main():
    ...
    parser.add_argument('-src', '--source', default="suppression_rate_sorted.csv", type=str, help="CSV file to make heatmap from.")
    ...
    parser.add_argument('-x_label', '--X_LABEL', default="Heterozygotes survival rate", type=str, help="Label for the x axis.")#Low density growth rate
