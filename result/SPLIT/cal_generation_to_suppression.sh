...
echo "drive_fitness,drop_ratio,drive_efficiency,low_density_growth_rate,density_growth_curve,weeks_to_suppression,type"
...
do
    ...
    drop_ratio=`echo $last_rows | cut -f 11 -d ',' | head -n 1`
    low_density_growth_rate=`echo $last_rows | cut -f 12 -d ',' | head -n 1`
    density_growth_curve=`echo $last_rows | cut -f 13 -d ',' | head -n 1 | cut -f 1 -d ' '`
    type=`echo $file | cut -f 1 -d '_'`
    ...
    echo "$fitness,$drop_ratio,$drive_efficiency,$low_density_growth_rate,$density_growth_curve,$generation,$type"
